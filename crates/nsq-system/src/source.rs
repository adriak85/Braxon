...
impl SourceTree {
    /// Walk the entire repository. Nothing is silently ignored except .git
    /// internals and build output that is conventionally generated outside the
    /// source tree (.git, target). Every source artifact becomes an NSQ node.

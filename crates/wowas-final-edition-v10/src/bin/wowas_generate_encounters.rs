...
fn main() -> std::io::Result<()> {
    ...
    let source_profiles =
        read_source_profiles(&root.join(".non_wowas_source_inspiration_lattice.file"));
    let books = read_book_plans(
        &canon.join("canonical_story_tree/characters/02_CHARACTER_PLACEMENT_BY_BOOK.md"),
    );
...
fn parse_book_filter(args: &[String]) -> Option<usize> {
    for i in 0..args.len() {
        if args[i] == "--book" {
            return args
                .get(i + 1)
                .and_then(|s| s.trim_start_matches('B').parse::<usize>().ok());
        }
...
fn review_all_characters(
    ...
    for c in &mut out {
        ...
        if !c.source_anchor.is_empty() {
            c.score += 15;
        }
        if c.role.contains("story") || c.role.contains("support") {
            c.score += 10;
        }
...
fn add_candidate(
    ...
    entry.score += score;
    if entry.source_anchor.is_empty() {
        entry.source_anchor = best_source_anchor(&entry.name, context, sources);
    }
...
fn absorb_generated_lattice(
    ...
    for line in lines {
        ...
        let score = col(&cols, &idx, "selection_score")
            .parse::<i32>()
            .unwrap_or(0);
        add_candidate(
            map,
...
fn read_creatures(path: &Path) -> Vec<Creature> {
    ...
    for line in lines {
        ...
        out.push(Creature {
            ...
            score: col(&cols, &idx, "selection_score")
                .parse::<i32>()
                .unwrap_or(0),
        });
    }
...
fn write_character_encounters(
    out_dir: &Path,
    books: &[BookPlan],
    candidates: &[CharacterCandidate],
    sources: &[SourceProfile],
    only_book: Option<usize>,
) -> std::io::Result<()> {
    ...
    for book in books
        .iter()
        .filter(|b| only_book.map(|x| x == b.num).unwrap_or(true))
    {
        let selected = select_best_for_book(book.num, candidates, 18);
        let mut core: Vec<CharacterCandidate> =
            selected.iter().filter(|c| c.is_core).cloned().collect();
        if core.is_empty() {
            core = selected.iter().take(3).cloned().collect();
        }
        let satellites: Vec<CharacterCandidate> =
            selected.iter().filter(|c| !c.is_core).cloned().collect();
        let fallback_satellites = if satellites.is_empty() {
            selected.clone()
        } else {
            satellites.clone()
        };
    ...
    let mut dark = BufWriter::new(File::create(
        out_dir.join("wowas_dark_transient_plot_threads.tsv"),
    )?);
    writeln!(
        dark,
        "book_num\tencounter_id\tcharacter\tplot_thread_obligation"
    )?;
...
fn write_character_row<W: Write>(
    out: &mut W,
    book: &BookPlan,
    id_num: usize,
    core: &CharacterCandidate,
    sat: &CharacterCandidate,
    sources: &[SourceProfile],
    coverage: &mut BTreeMap<(usize, String), usize>,
    dark_rows: &mut Vec<String>,
    rule: &str,
) -> std::io::Result<()> {
    let profiles = profiles_for(
        &format!(
            "{} {} {} {}",
            core.name, sat.name, core.source_anchor, sat.source_anchor
        ),
        sources,
    );
    ...
    let traits = profiles
        .iter()
        .take(3)
        .map(|p| format!("{} / {} / {}", p.function, p.relationship, p.power))
        .collect::<Vec<_>>()
        .join(" || ");
    let event = CHARACTER_EVENTS[id_num % CHARACTER_EVENTS.len()];
    let stakes = if sat.is_dark_transient {
        "dark transient match must enter plot continuity"
    } else {
        "relationship, pressure, humor, and future obligation"
    };
    let memorable = if sat.score > 150 {
        "high-score satellite gets recurring callback seed"
    } else {
        "small event must leave a visible mark"
    };
    let incorporation = if sat.is_dark_transient {
        plot_obligation(&profiles, sat)
    } else {
        "encounter may remain local unless later scene pressure calls it back".to_string()
    };
...
fn write_wildlife_encounters(
        ...
        .filter(|b| only_book.map(|x| x == b.num).unwrap_or(true))
    {
        ...
        for i in 0..4 {
            if creatures.is_empty() || selected.is_empty() {
                continue;
            }
        ...
        .filter(|b| only_book.map(|x| x == b.num).unwrap_or(true))
    {
        ...
        for i in 0..6 {
            ...
            writeln!(
                ...
                tsv(ch
                    .map(|x| x.name.as_str())
                    .unwrap_or("local desert satellite")),
                tsv(cr
                    .map(|x| x.name.as_str())
                    .unwrap_or("desert wildlife pending")),
                tsv(sp.map(|x| x.name.as_str()).unwrap_or("source pending")),
                tsv(pressure),
                tsv(route),
                tsv(&memorable),
                "yes"
...
fn write_review_summary(
    ...
    let mut out = BufWriter::new(File::create(
        out_dir.join("wowas_encounter_source_review.tsv"),
    )?);
    writeln!(out, "kind\tcount\tnote")?;
    writeln!(
        out,
        "source_inspiration_profiles\t{}\tnon-WoWaS hidden source lattice profiles loaded",
        sources.len()
    )?;
    ...
    let mut top = BufWriter::new(File::create(
        out_dir.join("wowas_best_character_picks.tsv"),
    )?);
    writeln!(
        top,
        "book_num\trank\tname\tscore\ttier\treason\tsource_anchor\tis_core\tis_dark_transient"
    )?;
    let mut by_book: BTreeMap<usize, Vec<&CharacterCandidate>> = BTreeMap::new();
    for c in candidates {
        by_book.entry(c.book).or_default().push(c);
    }
...
fn plot_obligation(profiles: &[&SourceProfile], sat: &CharacterCandidate) -> String {
    ...
    for p in profiles.iter().take(2) {
        if !p.dark_rule.is_empty() {
            bits.push(p.dark_rule.clone());
        }
        if !p.obligation.is_empty() {
            bits.push(p.obligation.clone());
        }
    }
    bits.join("; ")
}
...
fn is_dark_transient(name: &str, role: &str, reason: &str, anchor: &str) -> bool {
    ...
    let transient = s.contains("transient")
        || s.contains("temporary")
        || s.contains("encounter")
...
fn clean_name(s: &str) -> String {
    ...
    x.trim_matches(|c: char| c == '-' || c == '*' || c == '.' || c.is_whitespace())
        .trim()
        .to_string()
}
...
fn extract_markdown_names(text: &str) -> Vec<String> {
    ...
    for line in text.lines() {
        ...
        if t.starts_with("### ") || t.starts_with("## ") || t.starts_with("- ") {
            ...
            if cleaned.split_whitespace().count() <= 5 && cleaned.len() > 1 {
                out.push(cleaned);
            }
        }
    }

...
const ROLES: &[&str] = &[
    "field healer", "map seller", "tunnel witness", "corridor child", "ripper survivor",
    "sonomancy listener", "glyph apprentice", "glass forge worker", "orchard guard", "wolf-road scout",
    "market singer", "stone archivist", "faction runner", "bridge keeper", "garden watcher",
    "triage helper", "bookbinder", "cavern farmer", "cellar cook", "threshold mason",
];

const REGIONS: &[&str] = &[
    "willow-stone county", "glass orchard", "stone fen", "blue-light road", "ripper tunnel",
    "morrow market", "black-heart ridge", "last open gate", "neith edge", "pocket sanctuary",
    "xeth glyph yard", "cello field", "root vale", "ash river", "diamond breakland",
];

const HOUSE_PRESSURES: &[&str] = &[
    "Vayne", "Null-Heart", "Iron-Song", "Wren-Ash", "Thorn-Vayne", "Marso-len",
    "Rolzen-line", "Kyreal-pack", "Xethrolund-glyph", "Pip-no-path", "Mack-ground", "Dervish-glass",
];

const SOURCES: &[&str] = &[
    "Pip source orbit", "Rolzen source orbit", "Xethrolund source orbit", "Rylos source orbit",
    "Kyreal source orbit", "Thessa source orbit", "Kael source orbit", "Pael source orbit",
    "Soth source orbit", "Vellin source orbit", "Corrath source orbit", "Dervish source orbit",
];

const TIERS: &[&str] = &["story", "support", "recurring-background", "background", "deep-background"];

const CREATURE_FORMS: &[&str] = &[
    "moth-deer", "root hound", "glass heron", "basalt hare", "orchard eel", "lantern fox",
    "cinder elk", "moss bear", "willow crab", "stonefin bird", "thorn whale", "ash otter",
    "ribbon serpent", "mirror lynx", "fern mantis", "marrow crane", "quartz ram", "song beetle",
];

const TRAITS: &[&str] = &[
    "weird", "gross", "beautiful", "horrific", "elegant", "chaotic", "sanitized", "verdant",
    "unibreccianated", "unbrictionable", "resonant", "glyph-touched", "diamond-scarred", "cello-woken",
];

const ECOLOGY: &[&str] = &[
    "pollinator", "carrion recycler", "warning species", "healing symbiote", "apex grazer",
    "burrow architect", "memory mimic", "root spreader", "glass cleaner", "river filter",
    "gate sentinel", "song amplifier", "soil restorer", "weather listener", "pack companion",
];
...
fn reviewed_name(seed: &str, pressure: &str, source: &str, role: &str, region: &str) -> (String, u16) {
...
fn main() -> std::io::Result<()> {
    ...
    for i in 1..=CHARACTER_TARGET {
        ...
        let (mut name, mut score) = loop {
            ...
            if used_names.insert(candidate.0.clone()) || seed_count > 20 { break candidate; }
            seed_count += 1;
        };
    ...
    for i in 1..=CREATURE_TARGET {
        ...
        let danger = match i % 5 { 0 => "gentle", 1 => "watchful", 2 => "hazardous", 3 => "predatory", _ => "sacred-dangerous" };
        ...
        let (mut name, mut score) = loop {
            ...
            if used_creatures.insert(candidate.0.clone()) || seed_count > 20 { break candidate; }
            seed_count += 1;
        };

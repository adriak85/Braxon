...
impl NsqNativeBus {
    ...
    pub fn decide(&mut self, intent: &NsqIntent) -> NsqIntentDecision {
        ...
        if intent.narrative && intent.capability.starts_with("system.") {
            return NsqIntentDecision {
                intent_id: intent.intent_id.clone(),
                outcome: IntentOutcome::Rejected,
                reason: "narrative intent cannot mutate system capability".to_string(),
                owner_surface: None,
            };
        }
        let owner = self.surfaces.values().find(|surface| {
            surface.active
                && intent
                    .target_addresses
                    .iter()
                    .all(|address| address.starts_with(&surface.address_prefix))
        });
        let Some(owner) = owner else {
            return NsqIntentDecision {
                intent_id: intent.intent_id.clone(),
                outcome: IntentOutcome::Deferred,
                reason: "no active council surface owns the target address".to_string(),
                owner_surface: None,
            };
        };
        if intent
            .target_addresses
            .iter()
            .any(|address| self.leases.contains_key(address))
        {
            return NsqIntentDecision {
                intent_id: intent.intent_id.clone(),
                outcome: IntentOutcome::Queued,
                reason: "target address is held by another piston lease".to_string(),
                owner_surface: Some(owner.surface_id.clone()),
            };
        }
        self.generation += 1;
        for address in &intent.target_addresses {
            self.leases.insert(
                ...
                AddressLease {
                    ...
                    owner_surface: owner.surface_id.clone(),
                    intent_id: intent.intent_id.clone(),
        ...
        NsqIntentDecision {
            ...
            reason: "NSQ address lease acquired".to_string(),
            owner_surface: Some(owner.surface_id.clone()),
        }
    }
    ...
    pub fn advance_piston(&mut self, intent_id: &str, phase: PistonPhase) -> Result<(), String> {
        ...
        for lease in self
            .leases
            .values_mut()
            .filter(|lease| lease.intent_id == intent_id)
        {
            lease.phase = phase.clone();
            found = true;
        }
        if !found {
            return Err("intent has no active piston lease".to_string());
        }
        if phase == PistonPhase::Release {
            self.leases.retain(|_, lease| lease.intent_id != intent_id);
        }
        Ok(())
    }
    ...
    pub fn activate(&mut self, surface_id: &str) -> Result<(), String> {
        let surface = self
            .surfaces
            .get_mut(surface_id)
            .ok_or_else(|| "unknown council surface".to_string())?;
        surface.active = true;
        Ok(())
    }
    ...
    pub fn daydream(
        &self,
        workload: DaydreamWorkload,
        system_intent_pending: bool,
    ) -> Result<DaydreamWorkload, String> {
        if workload.source != "wowas_narrative"
            || !workload.interruptible
            || workload.max_steps == 0
        {
            return Err(
                "daydream workload must be bounded, interruptible, and narrative-sourced"
                    .to_string(),
            );
        }
...
mod tests {
    ...
    fn council() -> Vec<CouncilSurface> {
        (0..10)
            .map(|index| CouncilSurface {
                ...
                role: if index < 6 { "brain" } else { "sensory" }.to_string(),
    ...
    fn intent(id: &str, address: &str) -> NsqIntent {
        NsqIntent {
            ...
            capability: "system.reconstruct".to_string(),
            gradient: [0.0; 8],
            target_addresses: vec![address.to_string()],
    ...
    fn piston_prevents_same_space_override() {
        ...
        assert_eq!(
            bus.decide(&intent("a", "council/0/state")).outcome,
            IntentOutcome::Accepted
        );
        assert_eq!(
            bus.decide(&intent("b", "council/0/state")).outcome,
            IntentOutcome::Queued
        );
    ...
    fn daydream_yields_to_system_intent() {
        ...
        let workload = DaydreamWorkload {
            workload_id: "dream-1".to_string(),
            source: "wowas_narrative".to_string(),
            interruptible: true,
            max_steps: 8,
            yielded_to_system_intent: false,
        };
        assert!(
            bus.daydream(workload, true)
                .unwrap()
                .yielded_to_system_intent
        );
    }
}

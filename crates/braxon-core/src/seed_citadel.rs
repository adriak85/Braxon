        ...
        let projections = self
            ...
            .map(|section| {
                (
                    ...
                    (universal_id % 4_294_967_291) as u32,
                )
            })
            .collect();
        UniversalToken {
...
pub fn build_seed_plan(
    seed_id: &str,
    logical_parameter_bits: u128,
    logical_state_bits: u128,
...
pub fn materialize_window(plan: &SeedMaterializationPlan, start: u128, bits: u64) -> CitadelState {
    ...
    bytes.extend_from_slice(plan.seed_id.as_bytes());
    bytes.extend_from_slice(&start.to_le_bytes());
    bytes.extend_from_slice(&resident.to_le_bytes());
    bytes.extend_from_slice(plan.tokenizer.version.as_bytes());
    CitadelState {
        ...
        parameter_digest: stable_bytes(&bytes),

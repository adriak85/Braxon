...
impl CitadelBit {
    ...
    pub fn check_in(&mut self, input: Vec<NSQSlot>) {
        if self.state == CitadelBitState::Destabilizing {
            self.release_and_reapproach();
        }
        self.instructions.extend(input);
    ...
    pub fn is_live(&self) -> bool {
        !self.instructions.is_empty() && self.state != CitadelBitState::Destabilizing
    }
    pub fn pressure_sum(&self) -> u64 {
        self.instructions
            .iter()
            .flat_map(|s| s.body.iter())
            .filter(|l| l.charge == Charge::Positive)
            .map(|l| l.position)
            .sum()
    }
}

...
fn main() {
    ...
    let input = if input.trim().is_empty() { "What has not been seen yet?" } else { input.as_str() };
    ...
    println!("logical_complete={} resident_slots={}", materialized.logical_complete, materialized.slots.len());
}

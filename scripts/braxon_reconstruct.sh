...
require_contracts() {
  for path in \
    ...
    tools/toolchain/validate_toolchain_contracts.mjs; do
...
source_edge() {
  [ "${BRAXON_ALLOW_SOURCE_EDGE_NETWORK:-0}" = "1" ] || fail "source-edge acquisition is opt-in; set BRAXON_ALLOW_SOURCE_EDGE_NETWORK=1 after reviewing pinned source and licenses"
  cd "$ROOT"
  ...
  for path in state/full_android_language_toolchain/src/rust state/full_android_language_toolchain/src/cpython; do
    [ -d "$ROOT/$path/.git" ] || fail "pinned source edge did not materialize: $path"
  done
  notice "pinned_rust_and_cpython_source_edges_materialized=true"
}
...
source_build() {
  ...
  for path in state/full_android_language_toolchain/src/llvm-project state/full_android_language_toolchain/src/rust state/full_android_language_toolchain/src/cpython; do
    [ -d "$ROOT/$path" ] && [ "$(find "$ROOT/$path" -mindepth 1 -maxdepth 1 -print -quit)" ] || fail "source-build prerequisite is absent: $path; materialize the pinned source edge before build"
  done
...
case "$MODE" in
  status)
    require_contracts
    notice "root=$ROOT"
    notice "source_build_ready=false"
    notice "next=run preflight, source-edge (when approved), then source-build"
    ;;
  preflight)
    require_contracts
    target_preflight
  ...
  source-build)
    ...
    source_build
    ;;
  *) fail "usage: $0 [status|preflight|source-edge|verify|calibrate|source-build]" ;;
esac

...
CON=FL/'generated_character_flavor_constraints.tsv'; MECH=FL/'source_media_mechanism_catalog.tsv'; DYN=FL/'authored_dynamics_lattice.tsv'; CHAR=FL/'authored_character_flavor_lattice.tsv'

def tsv(p):
 with p.open(newline='',encoding='utf-8',errors='replace') as f:return list(csv.DictReader(f,delimiter='\t'))
def main():
 failures=[]; checks=[]
 for p in (META,CON,MECH,DYN,CHAR):
 ...
 meta=tsv(META); con=tsv(CON); mechs=tsv(MECH); dyn=tsv(DYN); chars=tsv(CHAR)

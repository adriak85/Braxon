...
def main():
 ...
 for c in chars:
  ...
  rows.append({'canonical_id':cid,'canonical_name':name,'aliases':vals(c.get('aliases',[])),'tier':c.get('tier',''),'pronouns':c.get('pronouns',''),'age':c.get('age',''),'species':c.get('species',''),'first_book':c.get('first_book',''),'last_book':c.get('last_book',''),'faction':c.get('faction',''),'role':c.get('role',''),'locked_traits':vals(c.get('locked_traits',[])),'shadow':c.get('shadow',''),'magic':vals(c.get('magic',[])),'sources':vals(c.get('sources',[])),'voice_constraint':t.get('role_summary','') or t.get('activity_mode',''),'pressure_response':t.get('peril_state',''),'dynamic_function':t.get('book_function_notes',''),'relationship_functions':'','timeline_fields':json.dumps(t,sort_keys=True),'source_registry':'06_CHARACTER_REGISTRY.json + wowas_character_timeline_lattice_UNIFIED_v14.tsv','prose_policy':'metadata only; no generated prose'})
 ...
 with (OUT/'authored_dynamics_lattice.tsv').open('w',newline='',encoding='utf-8') as f:
  w=csv.DictWriter(f,fieldnames=list(dyn[0]) if dyn else ['dynamic_id'],delimiter='\t');w.writeheader();w.writerows(dyn)

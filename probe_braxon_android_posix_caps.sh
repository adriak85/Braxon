...
OUT="state/full_android_language_toolchain/braxon_android_posix_capability_probe_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUT"
...
clang -Wall -Wextra -std=c11 "$OUT/probe.c" -o "$OUT/probe" 2>&1 | tee "$OUT/compile.log"
"$OUT/probe" 2>&1 | tee "$OUT/run.log"

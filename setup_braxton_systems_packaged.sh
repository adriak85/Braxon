...
ROOT="$HOME/Braxon"
BRANCH="braxton-systems"
PACKAGED="$ROOT/packaged"
TC="$ROOT/state/full_android_language_toolchain"
SRC="$TC/source_forge"
STAMP="$(date +%Y%m%d_%H%M%S)"
OUT="$TC/setup_braxton_systems_packaged_$STAMP.log"
JOBS="${JOBS:-7}"

cd "$ROOT"

{
  echo "=== Braxton Systems packaged staging ==="
  date
...
PY
  ...
  cat > "$PACKAGED/identity/BRAXTON_SYSTEMS_IDENTITY.md" <<'EOF'
# Braxton Systems Identity
...
EOF
  ...
  tar -czf "$PACKAGED/tarballs/braxton-systems-review-staging-$STAMP.tar.gz" -C "$ROOT" packaged
  sha256sum "$PACKAGED/tarballs/braxton-systems-review-staging-$STAMP.tar.gz" > "$PACKAGED/tarballs/braxton-systems-review-staging-$STAMP.tar.gz.sha256"
  ...
  echo "tarball=$PACKAGED/tarballs/braxton-systems-review-staging-$STAMP.tar.gz"
  echo "log=$OUT"
} 2>&1 | tee "$OUT"

ln -sf "$OUT" "$TC/setup_braxton_systems_packaged_latest.log"

...
const galaxyA17Package = parsed.get('config/toolchains/galaxy_a17_dense_artifact_package_contract.json');
const capacityProfile = parsed.get('config/toolchains/termux_android_aarch64_capacity_profile.json');
...
for (const scopeId of ['llvm-source-edge', 'llvm-aarch64-source-build', 'galaxy-a17-artifact-package', 'rust-edge-nightly', 'quickjs', 'nodejs', 'openjdk', 'jni-bridge', 'aarch64-assembly', 'optimization-proof']) {
...
const galaxyA17Scope = semanticBuild.scopes.find((item) => item.id === 'galaxy-a17-artifact-package');
if (!galaxyA17Scope || galaxyA17Scope.executor_mode !== 'artifact-package' || galaxyA17Scope.target_state !== 'TARGET_BUILD_PENDING' || !galaxyA17Scope.allowed_actions.includes('execute') || !galaxyA17Scope.required_paths.includes('scripts/toolchains/capture_galaxy_a17_dense_artifact_package.sh') || !galaxyA17Scope.output_paths?.includes('state/full_android_language_toolchain/artifact_packages/galaxy_a17_aarch64/artifact_manifest.json')) throw new Error('Galaxy A17 artifact package scope is invalid');
if (!semanticBuild.chain_reactions.some((reaction) => reaction.upstream_scope === 'llvm-aarch64-source-build' && reaction.downstream_scope === 'galaxy-a17-artifact-package' && reaction.condition === 'executed_with_declared_outputs_watermarked')) throw new Error('Galaxy A17 artifact package is not KSR-queued after verified LLVM output');
if (galaxyA17Package.schema !== 'braxon.toolchain.galaxy_a17_dense_artifact_package_contract.v1' || galaxyA17Package.capability !== 'feature:toolchain.galaxy_a17_artifact_package' || galaxyA17Package.target?.device_family !== 'Samsung Galaxy A17' || galaxyA17Package.target?.required_abi !== 'aarch64-linux-android' || galaxyA17Package.target?.reported_marketed_memory_gib !== 4 || galaxyA17Package.target?.reported_internal_storage_gib !== 128 || galaxyA17Package.target?.observed_microarchitecture?.little_cluster?.core !== 'Cortex-A55' || galaxyA17Package.target?.observed_microarchitecture?.little_cluster?.count !== 6 || galaxyA17Package.target?.observed_microarchitecture?.big_cluster?.core !== 'Cortex-A78' || galaxyA17Package.target?.observed_microarchitecture?.big_cluster?.count !== 2 || !galaxyA17Package.target?.observed_microarchitecture?.observed_instruction_features?.includes('asimddp') || galaxyA17Package.package?.compression !== 'xz_deterministic_maximum_density' || galaxyA17Package.package?.runtime_activation_automatic || galaxyA17Package.package?.mixed_core_density_policy?.runtime_code_generation !== 'must_remain_executable_on_both_observed_A55_and_A78_clusters' || galaxyA17Package.package?.mixed_core_density_policy?.a78_only_codegen_before_per_cluster_probe || galaxyA17Package.package?.mixed_core_density_policy?.physical_build_parallelism !== 'JOBS=1_until_measured_thermal_memory_and_throughput_evidence_allows_change' || galaxyA17Package.clone_admission?.fallback !== 'KSR_authorized_source_rebuild' || galaxyA17Package.publication_boundary?.current_package_state !== 'TARGET_BUILD_PENDING') throw new Error('Galaxy A17 artifact package contract is invalid');
if (capacityProfile.environment_observation?.device_provenance?.device_family !== 'Samsung Galaxy A17' || capacityProfile.environment_observation?.device_provenance?.reported_marketed_memory_gib !== 4 || capacityProfile.environment_observation?.device_provenance?.reported_internal_storage_gib !== 128) throw new Error('Galaxy A17 observed capacity profile is invalid');
...
const galaxyA17Distribution = graph.nodes.find((item) => item.id === 'galaxy_a17_dense_artifact_distribution');
if (!galaxyA17Distribution || !galaxyA17Distribution.depends_on.includes('repository_built_tool_dispatch') || !galaxyA17Distribution.depends_on.includes('ksr_semantic_build_dialect') || !galaxyA17Distribution.operation.includes('galaxy-a17-artifact-package') || !galaxyA17Distribution.blocked_until?.includes('large-artifact transport is configured before any oversized archive publication')) throw new Error('Galaxy A17 dense artifact distribution graph node is invalid');
const calibrationNode = graph.nodes.find((item) => item.id === 'termux_nsq_calibration_and_recovery');
if (!calibrationNode || !calibrationNode.depends_on.includes('repository_built_tool_dispatch') || !calibrationNode.depends_on.includes('galaxy_a17_dense_artifact_distribution')) throw new Error('Termux calibration is not bound to the Galaxy A17 repository-built artifact distribution');
